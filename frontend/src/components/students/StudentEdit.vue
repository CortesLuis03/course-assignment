              ...
              <div class="form-group row mb-3 g-2">
                <div class="col-3">
                  ...
                  <input type="text" class="form-control" placeholder="Identification Number" id="dni"
                    v-model="student.std_dni" disabled>
                </div>
                <div class="col-3">
...
export default {
  ...
  methods: {
    ...
    saveChanges() {
      axios.put(`http://localhost:8000/api/student/update/${this.student.id}`, this.student)
        .then(
          ({ data }) => {
            console.log(data)
    ...
    StudentInfo() {
      const url = `http://localhost:8000/api/student/${this.$route.params.id}`;
      axios.get(url).then(
        ({ data }) => {
          // console.log(data);
          this.student = data;
